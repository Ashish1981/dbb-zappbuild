...
def isGitDir(String dir) {
	String cmd = "/etc/git/bin/git $dir rev-parse --is-inside-work-tree"
...
def getCurrentGitBranch(String gitDir) {
	String cmd = "/etc/git/bin/git $gitDir rev-parse --abbrev-ref HEAD"
...
def getCurrentGitDetachedBranch(String gitDir) {
	String cmd = "/etc/git/bin/git $gitDir show -s --pretty=%D HEAD"
	StringBuffer gitBranch = new StringBuffer()
	StringBuffer gitError = new StringBuffer()
...
def isGitDetachedHEAD(String gitDir) {
	String cmd = "/etc/git/bin/git $gitDir status"
...
def getCurrentGitHash(String gitDir) {
	String cmd = "/etc/git/bin/git $gitDir rev-parse HEAD"
...
def getFileCurrentGitHash(String gitDir, String filePath) {
	String cmd = "/etc/git/bin/git $gitDir rev-list -1 HEAD " + filePath
	StringBuffer gitHash = new StringBuffer()
	StringBuffer gitError = new StringBuffer()
...
def getCurrentGitUrl(String gitDir) {
	String cmd = "/etc/git/bin/git $gitDir config --get remote.origin.url"
...
def getPreviousGitHash(String gitDir) {
	String cmd = "/etc/git/bin/git $gitDir --no-pager log -n 1 --skip=1"
...
def getChangedFiles(String gitDir, String baseHash, String currentHash) {
	String cmd = "/etc/git/bin/git $gitDir --no-pager diff --name-status $baseHash $currentHash"
...
def getCurrentChangedFiles(String gitDir, String currentHash, String verbose) {
	if (verbose) println "** Running git command: /etc/git/bin/git $gitDir show --pretty=format: --name-status $currentHash"
	String cmd = "/etc/git/bin/git $gitDir show --pretty=format: --name-status $currentHash"
